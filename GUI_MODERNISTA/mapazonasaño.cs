...
{
    public partial class mapazonasaño : UserControl
    {
        ...
        private async void UserControl1_LoadAsync(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conexi = conexion.conectarbd())
            {
                ...
                {
                    comando = new SqlCommand(string.Format(
                        "SELECT t1.ID_MEDIDOR, t2.DEPARTAMENTO, t2.ZONA,t2.DIRECCION , t2.MUNICIPIO  FROM PREDIO t2 inner join MEDIDOR M on t2.ID_PREDIO = M.ID_PREDIO inner join VARIABLES t1 on t1.ID_MEDIDOR = M.ID_MEDIDOR WHERE  t2.MUNICIPIO ='" + datos.ciudad + "' AND  DATEPART (YEAR, t1.FECHA)>= " + datos.FechaInicio + " AND DATEPART (YEAR, t1.FECHA)<= " + datos.FechaFin + ""), conexi);
                }
                if (datos.ciudad == "" && datos.departamento != "" && datos.zona == "")
                {
                    comando = new SqlCommand(string.Format(
                ...
                {
                    comando = new SqlCommand(string.Format(
                        "SELECT t1.ID_MEDIDOR, t2.DEPARTAMENTO, t2.ZONA,t2.DIRECCION , t2.MUNICIPIO  FROM PREDIO t2 inner join MEDIDOR M on t2.ID_PREDIO = M.ID_PREDIO inner join VARIABLES t1 on t1.ID_MEDIDOR = M.ID_MEDIDOR WHERE  t2.ZONA = '" + datos.zona + "' AND  DATEPART (YEAR, t1.FECHA)>= " + datos.FechaInicio + " AND DATEPART (YEAR, t1.FECHA)<= " + datos.FechaFin + ""), conexi);
                }
                if (datos.ciudad != "" && datos.departamento != "" && datos.zona == "")
                {
                    comando = new SqlCommand(string.Format(
                ...
                {
                    comando = new SqlCommand(string.Format(
                        "SELECT  t1.ID_MEDIDOR, t2.DEPARTAMENTO, t2.ZONA,t2.DIRECCION , t2.MUNICIPIO FROM PREDIO t2 inner join MEDIDOR M on t2.ID_PREDIO = M.ID_PREDIO inner join VARIABLES t1 on t1.ID_MEDIDOR = M.ID_MEDIDOR WHERE  t2.MUNICIPIO = '" + datos.ciudad + "' AND t2.ZONA = '" + datos.zona + "' AND  DATEPART (YEAR, t1.FECHA)>= " + datos.FechaInicio + " AND DATEPART (YEAR, t1.FECHA)<= " + datos.FechaFin + ""), conexi);
                }
                if (datos.ciudad != "" && datos.departamento != "" && datos.zona == "")
                {
                    comando = new SqlCommand(string.Format(
