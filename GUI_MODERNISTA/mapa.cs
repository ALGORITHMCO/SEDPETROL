        ...
        private async void UserControl1_LoadAsync(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conexi = conexion.conectarbd())
            {
                ...
                SqlCommand comando = new SqlCommand(string.Format(
                    "SELECT  t2.DEPARTAMENTO, t2.ZONA,t2.DIRECCION , t2.MUNICIPIO  FROM PREDIO t2 inner join MEDIDOR M on t2.ID_PREDIO = M.ID_PREDIO inner join VARIABLES t1 on t1.ID_MEDIDOR = M.ID_MEDIDOR WHERE M.ID_MEDIDOR = '" + datos.ID_MEDIDOR + "'  AND t1.FECHA >= '" + datos.FechaInicio + "' and t1.FECHA <= '" + datos.FechaFin + "'"), conexi);
                ...
                while (reader.Read())
                {
                    ...
                    direccion1 = reader.GetString(2);
                    //ConsultareportesyanalisisIN dire = new ConsultareportesyanalisisIN(panel);
                    var address = reader.GetString(2) + reader.GetString(3) + reader.GetString(0);
                    ...
                    if (direccion2 != direccion1)
                    {
                        using (var client = new HttpClient())
                        {
                            ...
                            foreach (var item in root.results)
                            {
                                ...
                                marker.ToolTipText = String.Format(direccion1);
